...
def build_measurement_scale_tensor(
    neutral_measurements: torch.Tensor,
    normalization_eps: float,
) -> torch.Tensor:
    """
    Convert neutral measurements into stable normalization scales.

    This keeps PC selection and optimization loss on the same footing by
    expressing residuals in relative units rather than raw measurement units.
    """
    return torch.clamp(torch.abs(neutral_measurements), min=float(normalization_eps))
...
def select_shape_pcs_for_experiment(
    ...
    target_scale = build_measurement_scale_tensor(
        neutral_measurements=neutral_measurements,
        normalization_eps=normalization_eps,
    )
    preserve_scale = build_measurement_scale_tensor(
        neutral_measurements=neutral_preserve_measurements,
        normalization_eps=normalization_eps,
    )
    ...
    return {
        "face_width_region": {
            ...
            "preserve_weight_overrides": {
                "chin_center_to_nose_bridge": 1.4,
                "lower_face_height": 1.4,
        ...
        "face_height_region": {
            "relative_controls": {
                "chin_center_to_nose_bridge": -0.12,
                "lower_face_height": -0.10,
            },
            "measurement_weight_overrides": {
                "chin_center_to_nose_bridge": 2.4,
                "lower_face_height": 2.2,
            },
            "preserve_measurements": [
                "face_width",
                "midface_width",
        ...
        "nose_bridge_width_only": {
            "relative_controls": {
                "nose_bridge_width_proxy": -0.10,
            },
            "measurement_weight_overrides": {
                "nose_bridge_width_proxy": 3.0,
            },
            "preserve_measurements": [
                "nose_width",
                "nose_height",
        ...
        "philtrum_region": {
            "relative_controls": {
                "philtrum_height_proxy": -0.05,
            },
            "measurement_weight_overrides": {
                "philtrum_height_proxy": 2.8,
            },
            "preserve_measurements": [
                "mouth_width",
                "inner_mouth_width",
            ...
            "preserve_weight_overrides": {
                "mouth_width": 1.5,
                "inner_mouth_width": 1.4,
                "mouth_opening_height": 1.2,
                "mouth_to_chin_height": 1.1,
                "nose_width": 1.0,
                "nose_height": 1.0,
            },
        },
        ...
        "eyes_shape": {
            ...
            "measurement_weight_overrides": {
                "left_eye_opening_height": 2.8,
                "right_eye_opening_height": 2.8,
            },
            "preserve_measurements": [
                "left_eye_width",
                "right_eye_width",
        ...
        "brow_arch": {
            ...
            "measurement_weight_overrides": {
                "left_brow_to_eye_distance": 2.4,
                "right_brow_to_eye_distance": 2.4,
            },
            "preserve_measurements": [
                "brow_width_left",
                "brow_width_right",
...
def apply_preserve_anchor_defaults(
    config: Dict[str, object],
    anchor_measurements: List[str],
    default_weight: float = 0.9,
) -> Dict[str, object]:
    """
    Add a stable set of preserve anchors for multi-trait dataset fits.

    We only add anchors that are not already target measurements.
    """
    updated = {
        "relative_controls": dict(config["relative_controls"]),  # type: ignore
        "measurement_weight_overrides": dict(config["measurement_weight_overrides"]),  # type: ignore
    ...
    target_names = set(updated["relative_controls"].keys())

    for measurement_name in anchor_measurements:
        if measurement_name in target_names:
            continue

        if measurement_name not in updated["preserve_measurements"]:
            updated["preserve_measurements"].append(measurement_name)

        updated["preserve_weight_overrides"].setdefault(
            measurement_name,
...
def build_measurement_conflict_report(
    named_scaled_configs: List[Tuple[str, float, Dict[str, object]]],
    eps: float = 1e-5,
) -> List[Dict[str, object]]:
    """
    Report cases where multiple active regions push the same measurement
    in opposite directions before the final merge.
    """
    per_measurement: Dict[str, List[Dict[str, float | str]]] = {}

    for region_name, score, cfg in named_scaled_configs:
        relative_controls = cfg["relative_controls"]  # type: ignore
        for measurement_name, value in relative_controls.items():
            per_measurement.setdefault(str(measurement_name), []).append(
                {
                    "region": region_name,
                    "control_score": float(score),
                    "offset": float(value),
                }
            )
    ...
    for measurement_name, items in per_measurement.items():
        positive_items = [item for item in items if float(item["offset"]) > eps]
        negative_items = [item for item in items if float(item["offset"]) < -eps]

        if not positive_items or not negative_items:
            continue

        net_offset = float(sum(float(item["offset"]) for item in items))
        conflicts.append(
            {
                "measurement": measurement_name,
                "net_offset": net_offset,
                "positive_sources": positive_items,
                "negative_sources": negative_items,
            }
        )

    conflicts.sort(key=lambda item: abs(float(item["net_offset"])))
    return conflicts


def apply_control_group_budgets(
    control_scores: Dict[str, float],
) -> Tuple[Dict[str, float], List[Dict[str, object]]]:
    """
    Keep multi-trait dataset rows from overdriving FLAME when many coupled
    regions activate together.

    These budgets are intentionally conservative for dataset-mode synthesis:
    they trade some trait strength for a more stable, believable head shape.
    """
    adjusted = {name: float(score) for name, score in control_scores.items()}
    ...
    dampings: List[Dict[str, object]] = []

    for group_name, members in group_members.items():
        active_members = [
            name for name in members
            if abs(adjusted.get(name, 0.0)) > 1e-8
        ]
        if not active_members:
            continue

        raw_total = float(sum(abs(adjusted[name]) for name in active_members))
    ...
    return merge_nested_settings(
        get_control_defaults("global_face"),
        {
            "pc_selection": {
                "top_k": 36,
                "candidate_pool_size": 100,
                "preserve_tradeoff": 0.65,
            },
            "optimization": {
                "lambda_reg": 8e-5,
                "learning_rate": 0.035,
                "steps": 900,
                "lambda_preserve": 0.75,
            },
            "enable_visualization": True,
        },
    )
...
def build_dataset_row_experiment_config(
    ...
    ignored_geometry_traits: Dict[str, str] = {}
    ...
    # nose bridge width -> isolate this so it does not also push nose width/height.
    if geometry_traits["nose_bridge_width"] == "wide":
        add_vote("nose_bridge_width_only", -0.55)
    elif geometry_traits["nose_bridge_width"] == "narrow":
        add_vote("nose_bridge_width_only", +0.55)

    # nostril width is currently deferred because the available proxy reuses
    # the exact same landmark pair as nose_width and causes false independence.
    if geometry_traits["nostril_width"] in {"narrow", "wide"}:
        ignored_geometry_traits["nostril_width"] = (
            f"{geometry_traits['nostril_width']} "
            "(deferred: current proxy shares the same landmark pair as nose_width)"
        )
    ...
    # eyebrow thickness is also deferred for now. The current landmark-only
    # geometry controls do not represent true brow fullness robustly.
    if geometry_traits["eyebrow_thickness"] in {"thin", "thick"}:
        ignored_geometry_traits["eyebrow_thickness"] = (
            f"{geometry_traits['eyebrow_thickness']} "
            "(deferred: brow fullness is not a robust landmark-only geometry target)"
        )

    # brow arch
    if geometry_traits["eyebrow_arch"] == "high":
        add_vote("brow_arch", +0.65)
    elif geometry_traits["eyebrow_arch"] == "slight":
        add_vote("brow_arch", -0.35)
    ...
    # philtrum depth is isolated from mouth width so those two traits
    # no longer cancel each other through one shared mouth region.
    if geometry_traits["philtrum_depth"] == "shallow":
        add_vote("philtrum_region", +0.40)
    ...
    # ------------------------------------------------------------
    raw_control_scores: Dict[str, float] = {}
    for control_name, votes in control_votes.items():
        if len(votes) == 0:
            continue
        raw_control_scores[control_name] = float(
            np.clip(np.mean(votes), -1.25, 1.25)
        )

    control_scores, control_group_dampings = apply_control_group_budgets(
        raw_control_scores
    )
    ...
    for control_name, score in control_scores.items():
        if abs(score) < 1e-8:
            continue
        named_scaled_configs.append(
            (
                control_name,
                score,
                build_scaled_region_config(control_name, score),
            )
        )

    experiment_config = merge_custom_region_configs(
        [cfg for _, _, cfg in named_scaled_configs],
        max_abs_offset=0.20,
    )
    experiment_config = apply_preserve_anchor_defaults(
        experiment_config,
        anchor_measurements=[
            "face_width",
            "midface_width",
    ...
    metadata = {
        ...
        "control_group_dampings": control_group_dampings,
        "active_regions": [name for name, _, _ in named_scaled_configs],
...
def main():
    ...
    if dataset_mode and dataset_metadata is not None:
        ...
        print("\nRaw control votes:")
        for control_name, votes in dataset_metadata["control_votes"].items():  # type: ignore
            print(f"  - {control_name:24s} votes={votes}")

        print("\nRaw control scores before damping:")
        for control_name, score in dataset_metadata["raw_control_scores"].items():  # type: ignore
            print(f"  - {control_name:24s} raw_score={float(score):+.3f}")
        ...
        print("\nControl-group damping:")
        control_group_dampings = dataset_metadata["control_group_dampings"]  # type: ignore
        if control_group_dampings:
            for item in control_group_dampings:  # type: ignore
                print(
                    f"  - {item['group']:24s} "
                    f"raw_total={float(item['raw_total_abs_score']):.3f} "
                    f"cap={float(item['cap']):.3f} "
                    f"scale={float(item['scale']):.3f} "
        ...
        ignored_geometry_traits = dataset_metadata["ignored_geometry_traits"]  # type: ignore
        if ignored_geometry_traits:
            for trait_name, trait_value in ignored_geometry_traits.items():  # type: ignore
                print(f"  - {trait_name:24s} {trait_value}")
        else:
            print("  - none")

        print("\nRemaining measurement-level conflicts after mapping:")
        measurement_conflicts = dataset_metadata["measurement_conflicts"]  # type: ignore
        if measurement_conflicts:
            for item in measurement_conflicts:  # type: ignore
                measurement_name = str(item["measurement"])
                net_offset = float(item["net_offset"])
                print(
                    f"  - {measurement_name:24s} net_offset={net_offset:+.4f}"
                )
                positive_sources = item["positive_sources"]  # type: ignore
                negative_sources = item["negative_sources"]  # type: ignore
                for source in positive_sources:
                    print(
                        f"      + {source['region']:20s} "
                        f"score={float(source['control_score']):+.3f} "
                        f"offset={float(source['offset']):+.4f}"
                    )
    ...
    print("\nNormalization scales used by PC selection and loss:")
    for name, scale in zip(measurement_names, target_scale[0].tolist()):
        print(f"  - target  {name:21s} scale={scale:.6f}")
    for m, scale in zip(preserve_measurements, preserve_scale[0].tolist()):
        print(f"  - preserve {m['name']:20s} scale={scale:.6f}")
    ...
    for it in range(steps):
        ...
        normalized_diff_target = diff_target / target_scale
        weighted_sq_error_target = measurement_weights * (normalized_diff_target ** 2)
        loss_data = torch.mean(weighted_sq_error_target)

        diff_preserve = current_preserve_measurements - neutral_preserve_measurements
        normalized_diff_preserve = diff_preserve / preserve_scale
        weighted_sq_error_preserve = preserve_weights * (normalized_diff_preserve ** 2)
    ...
    neutral_abs_error_normalized = neutral_abs_error / target_scale
    final_abs_error_normalized = final_abs_error / target_scale
    preserve_abs_shift_normalized = preserve_abs_shift / preserve_scale
    ...
    for name, neutral_val, target_val, final_val, neutral_err, final_err, neutral_err_norm, final_err_norm in zip(
        measurement_names,
        neutral_measurements[0].tolist(),
        target_measurements[0].tolist(),
        ...
        neutral_abs_error_normalized[0].tolist(),
        final_abs_error_normalized[0].tolist(),
    ):
        ...
        print(
            ...
            f"neutral_rel_err={neutral_err_norm:.6f} "
            f"final_rel_err={final_err_norm:.6f} "
            f"improvement={improvement:+.6f}"
        )

    print("\nPreserved-measurement drift from neutral:")
    for m, neutral_val, final_val, shift, shift_norm in zip(
        preserve_measurements,
        neutral_preserve_measurements[0].tolist(),
        final_preserve_measurements[0].tolist(),
        preserve_abs_shift[0].tolist(),
        preserve_abs_shift_normalized[0].tolist(),
    ):
        print(
            f"  - {m['name']:28s} "
            f"neutral={neutral_val:.6f} "
    ...
    print(f"  Mean rel error vs target (neutral): {neutral_abs_error_normalized.mean().item():.6f}")
    print(f"  Mean rel error vs target (final):   {final_abs_error_normalized.mean().item():.6f}")
    print(f"  Mean preserved rel shift:           {preserve_abs_shift_normalized.mean().item():.6f}")
    ...
    metrics = {
        ...
        "relative_controls": relative_controls,
        "normalization_eps": normalization_eps,
        "target_scale": target_scale[0].tolist(),
        "preserve_scale": preserve_scale[0].tolist(),
        ...
        "neutral_abs_error_normalized": neutral_abs_error_normalized[0].tolist(),
        "final_abs_error_normalized": final_abs_error_normalized[0].tolist(),
        "preserve_abs_shift_normalized": preserve_abs_shift_normalized[0].tolist(),
        ...
        "mean_abs_error_neutral_normalized": float(
            neutral_abs_error_normalized.mean().item()
        ),
        "mean_abs_error_final_normalized": float(
            final_abs_error_normalized.mean().item()
        ),
        "mean_preserve_abs_shift_normalized": float(
            preserve_abs_shift_normalized.mean().item()
        ),
